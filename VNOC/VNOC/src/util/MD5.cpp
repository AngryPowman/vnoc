...
BOOL CMD5Checksum::GetMD5(LPCTSTR path, CString& md5)
{
	...
	int nLength = 0;				//number of bytes read from the file
	const int nBufferSize = 1024;	//checksum the file in blocks of 1024 bytes
	BYTE Buffer[nBufferSize] = {0};		//buffer for data read from the file
	...
	//checksum the file in blocks of 1024 bytes
	DWORD dwBytesRead = 0;
	while(ReadFile(hFile, Buffer, nBufferSize, &dwBytesRead, NULL) && dwBytesRead > 0)
	{
		MD5Checksum.Update(Buffer, dwBytesRead );
	}
	CloseHandle(hFile);

	//finalise the checksum and return it
	md5 =  MD5Checksum.Final();
	return TRUE;
}
