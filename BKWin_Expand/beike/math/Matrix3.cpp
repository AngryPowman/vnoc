...
bool Matrix3::fuzzyEq(const Matrix3& b) const {
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
            if (! BKMath::fuzzyEq(elt[r][c], b[r][c])) {
                return false;
            }
        }
    }
...
bool Matrix3::isOrthonormal() const {
    ...
    return 
        (BKMath::fuzzyEq(X.dot(Y), 0.0f) &&
         BKMath::fuzzyEq(Y.dot(Z), 0.0f) &&
         BKMath::fuzzyEq(X.dot(Z), 0.0f) &&
         BKMath::fuzzyEq(X.squaredMagnitude(), 1.0f) &&
         BKMath::fuzzyEq(Y.squaredMagnitude(), 1.0f) &&
         BKMath::fuzzyEq(Z.squaredMagnitude(), 1.0f));
}
...
void Matrix3::golubKahanStep (Matrix3& kA, Matrix3& kL,
    ...
    float fY = kA[0][0] - (BKMath::abs(fRoot1 - fT22) <=
                          BKMath::abs(fRoot2 - fT22) ? fRoot1 : fRoot2);
    float fZ = kA[0][1];
...
void Matrix3::singularValueDecomposition (Matrix3& kL, Vector3& kS,
    ...
    for (int i = 0; i < ms_iSvdMaxIterations; i++) {
        ...
        bool bTest1 = (BKMath::abs(kA[0][1]) <=
                       ms_fSvdEpsilon * (BKMath::abs(kA[0][0]) + BKMath::abs(kA[1][1])));
        bool bTest2 = (BKMath::abs(kA[1][2]) <=
                       ms_fSvdEpsilon * (BKMath::abs(kA[1][1]) + BKMath::abs(kA[2][2])));
...
float Matrix3::maxCubicRoot (float afCoeff[3]) {
    ...
    if ( fPoly < 0.0f ) {
        ...
        fX = BKMath::abs(afCoeff[0]);
        float fTmp = 1.0 + BKMath::abs(afCoeff[1]);

        if ( fTmp > fX )
            fX = fTmp;

        fTmp = 1.0 + BKMath::abs(afCoeff[2]);
...
void Matrix3::toAxisAngle (Vector3& rkAxis, float& rfRadians) const {
    ...
    rfRadians = BKMath::aCos(fCos);  // in [0,PI]
...
bool Matrix3::toEulerAnglesXYZ (float& rfXAngle, float& rfYAngle,
    ...
    if ( elt[0][2] < 1.0f ) {
        if ( elt[0][2] > -1.0f ) {
            rfXAngle = BKMath::aTan2( -elt[1][2], elt[2][2]);
            rfYAngle = (float) BKMath::aSin(elt[0][2]);
            rfZAngle = BKMath::aTan2( -elt[0][1], elt[0][0]);
            return true;
        } else {
            // WARNING.  Not unique.  XA - ZA = -atan2(r10,r11)
            rfXAngle = -BKMath::aTan2(elt[1][0], elt[1][1]);
...
bool Matrix3::toEulerAnglesXZY (float& rfXAngle, float& rfZAngle,
    ...
    if ( elt[0][1] < 1.0f ) {
        if ( elt[0][1] > -1.0f ) {
            rfXAngle = BKMath::aTan2(elt[2][1], elt[1][1]);
            rfZAngle = (float) asin( -elt[0][1]);
            rfYAngle = BKMath::aTan2(elt[0][2], elt[0][0]);
            return true;
        } else {
            // WARNING.  Not unique.  XA - YA = atan2(r20,r22)
            rfXAngle = BKMath::aTan2(elt[2][0], elt[2][2]);
...
bool Matrix3::toEulerAnglesYXZ (float& rfYAngle, float& rfXAngle,
    ...
    if ( elt[1][2] < 1.0 ) {
        if ( elt[1][2] > -1.0 ) {
            rfYAngle = BKMath::aTan2(elt[0][2], elt[2][2]);
            rfXAngle = (float) asin( -elt[1][2]);
            rfZAngle = BKMath::aTan2(elt[1][0], elt[1][1]);
            return true;
        } else {
            // WARNING.  Not unique.  YA - ZA = atan2(r01,r00)
            rfYAngle = BKMath::aTan2(elt[0][1], elt[0][0]);
...
bool Matrix3::toEulerAnglesYZX (float& rfYAngle, float& rfZAngle,
    ...
    if ( elt[1][0] < 1.0 ) {
        if ( elt[1][0] > -1.0 ) {
            rfYAngle = BKMath::aTan2( -elt[2][0], elt[0][0]);
            rfZAngle = (float) asin(elt[1][0]);
            rfXAngle = BKMath::aTan2( -elt[1][2], elt[1][1]);
            return true;
        } else {
            // WARNING.  Not unique.  YA - XA = -atan2(r21,r22);
            rfYAngle = -BKMath::aTan2(elt[2][1], elt[2][2]);
            rfZAngle = -(float)halfPi();
            rfXAngle = 0.0;
            return false;
        }
...
bool Matrix3::toEulerAnglesZXY (float& rfZAngle, float& rfXAngle,
    ...
    if ( elt[2][1] < 1.0 ) {
        if ( elt[2][1] > -1.0 ) {
            rfZAngle = BKMath::aTan2( -elt[0][1], elt[1][1]);
            rfXAngle = (float) asin(elt[2][1]);
            rfYAngle = BKMath::aTan2( -elt[2][0], elt[2][2]);
            return true;
        } else {
            // WARNING.  Not unique.  ZA - YA = -atan(r02,r00)
            rfZAngle = -BKMath::aTan2(elt[0][2], elt[0][0]);
            rfXAngle = -(float)halfPi();
            rfYAngle = 0.0f;
            return false;
        }
...
bool Matrix3::toEulerAnglesZYX (float& rfZAngle, float& rfYAngle,
    ...
    if ( elt[2][0] < 1.0 ) {
        if ( elt[2][0] > -1.0 ) {
            ...
            rfZAngle = -BKMath::aTan2(elt[0][1], elt[0][2]);
            rfYAngle = (float)halfPi();
            rfXAngle = 0.0f;
            return false;
        }
...
bool Matrix3::qLAlgorithm (float afDiag[3], float afSubDiag[3]) {
    ...
    for (int i0 = 0; i0 < 3; i0++) {
        ...
        for (iIter = 0; iIter < iMaxIter; iIter++) {
            ...
            for (i1 = i0; i1 <= 1; i1++) {
                float fSum = BKMath::abs(afDiag[i1]) +
                            BKMath::abs(afDiag[i1 + 1]);

                if ( BKMath::abs(afSubDiag[i1]) + fSum == fSum )
                    break;
            }
            ...
            for (int i2 = i1 - 1; i2 >= i0; i2--) {
                ...
                if (BKMath::abs(fTmp3) >= BKMath::abs(fTmp0)) {
                    fCos = fTmp0 / fTmp3;
