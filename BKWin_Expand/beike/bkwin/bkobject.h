...
#define BKWIN_FLOAT_ATTRIBUTE(attribname, varname, allredraw)       \
		if (attribname == strAttribName)                            \
		{															\
			float val = 0;											\
			sscanf(strValue, "%g", &val);							\
			varname = val;											\
...
// PNG = StringA
#define BKWIN_PNG_ATTRIBUTE(attribname, varname, allredraw)         \
        if (attribname == strAttribName)                            \
        {                                                           \
            varname = BkPngPool::Get(CA2T(strValue, CP_UTF8));      \
            hRet = allredraw ? S_OK : S_FALSE;                      \
        }                                                           \
        else                                                        \

// GIF = StringA
#define BKWIN_GIF_ATTRIBUTE(attribname, varname, allredraw)         \
        if (attribname == strAttribName)                            \
        {                                                           \
            varname = BkPngPool::GetGif(CA2T(strValue, CP_UTF8));   \
            hRet = allredraw ? S_OK : S_FALSE;                      \
        }                                                           \
        else                                                        \
...
// BkEffect From StringA Key
#define BKWIN_EFFECT_ATTRIBUTE(attribname, varname, allredraw)      \
    if (attribname == strAttribName)                                \
        {                                                           \
            varname = BkEffect::GetEffect(strValue);                \
            hRet = allredraw ? S_OK : S_FALSE;                      \
        }                                                           \
        else                                                        \

// ICON = StringA
#define BKWIN_ICON_ATTRIBUTE(attribname, varname, allredraw)        \
    if (attribname == strAttribName)                                \
    {                                                               \
        varname = BkIconPool::Get(strValue);                        \
        hRet = allredraw ? S_OK : S_FALSE;                          \
    }                                                               \
    else                                                            \
