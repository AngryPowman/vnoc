...
ResultCode CGlobalCenter::Log( CString file,CString str,BOOL info,BOOL endLine)
{
...
ResultCode CGlobalCenter::Logf( CString file,LPCTSTR str,... )
{
...
ResultCode CGlobalCenter::Lognf( CString file,LPCTSTR str,... )
{
	static TCHAR buffer[_LogFormatBufferSize];
	buffer[_LogFormatBufferSize-1] = L'\0';
	va_list vl;
...
ResultCode CGlobalCenter::SetLog( CString file,BOOL bDbgView/*=TRUE*/,BOOL bConsole/*=FALSE*/ )
{
	LogMap::iterator i;
	i = m_log.begin();
	if (i != m_log.end())
...
ResultCode CGlobalCenter::SetThreadName( CString name )
{
	if (name.IsEmpty())
	{
		return Result_Fail;
...
ResultCode CGlobalCenter::GetThreadName( DWORD threadID,CString& name )
{
...
ResultCode CGlobalCenter::LogPrefix( CString file,CString prefix,BOOL bAdd )
{
	blog::CBLog* log = _GetLogInstance(file);
	if (log)
	{
...
ResultCode CGlobalCenter::LogIndent( CString file,BOOL bAdd )
{
	blog::CBLog* log = _GetLogInstance(file);
	if (log)
	{
