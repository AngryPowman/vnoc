...
BOOL CConfig::Get(LPCTSTR path)
{
	...
	if (pConf)
	{
		ResultCode hr = pConf->Get(this->path,m_vec);
		return SUCCEEDED(hr);
	}
	return FALSE;
}
...
ResultCode CConfigImpl::Initialize( IModule* UpperFrame/*=NULL*/ )
{
	Global->Log(LogFile_Config,_T("初始化"));
	return Result_Success;
}

ResultCode CConfigImpl::UnInitialize()
{
...
ResultCode CConfigImpl::Run()
{
	Global->Log(LogFile_Config,_T("Config模块启动"));
	return Result_Success;
}

ResultCode CConfigImpl::Terminate()
{
	Global->Log(LogFile_Config,_T("Config模块中止"));
	return Result_Success;
}

ResultCode CConfigImpl::LoadConfigFromXML( LPCTSTR filePath )
{
	...
	return bRet? Result_Success:Result_Fail;
}
...
ResultCode CConfigImpl::SaveConfigToXML( LPCTSTR filePath )
{
	if (filePath == NULL)
	{
		filePath = m_filePath;
...
ResultCode CConfigImpl::Get(const ConfigPath& path,ConfigPtrVec& node )
{
...
ResultCode CConfigImpl::AddMonitor( ConfigPath path,IConfigMonitor* pMonitor )
{
	return Result_Success;
}

ResultCode CConfigImpl::RemoveMonitor( IConfigMonitor* pMonitor )
{
	return Result_Success;
}
