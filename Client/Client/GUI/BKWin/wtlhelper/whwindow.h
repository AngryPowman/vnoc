    ...
    void OnPaint(CDCHandle /*dc*/)
    {
        ...
        for (int i = 0; i < nItems; i++)
        {
            ...
            if (rcItem.left < rcItem.right)
                dc.DrawText(szText, (int)_tcslen(szText), &rcItem, uFormat);
...
protected:
    ...
    static UINT _RegisterTaskbarCreatedMessage()
    {
        ...
        pfn_ChangeWindowMessageFilter MyChangeWindowMessageFilter
            = (pfn_ChangeWindowMessageFilter)::GetProcAddress(::GetModuleHandle(_T("user32.dll")), "ChangeWindowMessageFilter");
